         ...
         sectionSize <- 100
         fullRange <- seq(3150, 3250, by = sectionSize)       # UK stations start below ~2800
               ...
               if (response$status == "ok") {  ### new line & brackets
                  ...
                  if (!is.null(response$data$city$geo[1]) & !is.null(response$data$city$geo[2])) {
                     if ((southExtent < checkCoords[2] & northExtent > checkCoords[2] & eastExtent > checkCoords[3] & westExtent < checkCoords[3]) | 
                       ...
                       # if statement to swap eastings with northings if necessary, happening on a four occasiions for Great Britain & Eire
         ...
         write_csv(aqiStationsDF0, path = '/home/jay/c3/citiesAQI/data/raw/ukEireAqiStations.csv', na = 'NA', append = TRUE, col_names = FALSE)
         stopTime <- Sys.time()
